...
from chnk import ChunkTrainer,PEFTrainer,GetCallBack,normalize_chunk_args,apply_gradient_checkpointing_strategy,peft_function,rebuild_layer
...
@dataclass
class ChunkFTArguments(ModelArguments):
    TaskType: str = field(
        default="SEQ_CLS",
        metadata={"help": ("TaskType should be consistent with PEFT TaskType" )},
    )
    peft_type: str = field(
        default=None,
    ...
    chunk_num: int = field(
        default=1,
        metadata={"help": ("the number of chunk for parameters" )},
    )
    ...
    chunk_update_interval: int = field(
        default=1,
        metadata={"help": "How many optimizer updates to apply on the selected chunk before switching to the next chunk."},
    )
    chunk_strategy: str = field(
        default="row",
        metadata={"help": ("optimizer strategy of ['row','column']" )},
    )
    chunk_tuning: bool = field(
        default=False,
        metadata={
            "help": (
                "ChunkFT optimization for LLMs"
            )
        },
    )
    gradient_checkpointing_layers: Optional[str] = field(
        default=None,
        metadata={"help": "Comma-separated transformer block indices/ranges for selective checkpointing, e.g. '9-11'."},
    )
    gradient_checkpointing_ratio: float = field(
        default=1.0,
        metadata={"help": "If 0<ratio<1 and no explicit layers are set, only checkpoint this fraction of transformer blocks."},
    )
    gradient_checkpointing_mode: str = field(
        default="tail",
        metadata={"help": "How to select checkpointed layers when using a ratio: one of ['tail','head','uniform']."},
    )
    chunk_tuning: bool = field(
        default=False,
...
def main():
    ...
    parser = HfArgumentParser((ChunkFTArguments, DataTrainingArguments, MyTrainingArguments))
    ...
    if data_args.task_name is not None:
        print(training_args.eval_metric)
        print("*"*10)
        metric = evaluate.load(training_args.eval_metric, data_args.task_name)
    else:
        metric = evaluate.load(training_args.eval_metric)
