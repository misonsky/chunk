...
from chnk import ChunkSeq2SeqTrainer,Seq2SeqTrainer,GetCallBack,normalize_chunk_args,apply_gradient_checkpointing_strategy,peft_function,rebuild_layer
...
class ChunkFTArguments(ModelArguments):
    ...
    gradient_checkpointing_layers: Optional[str] = field(
        default=None,
        metadata={"help": "Comma-separated transformer block indices/ranges for selective checkpointing, e.g. '18-23'."},
    )
...
def preprocess(
    ...
    input_ids = [torch.tensor(ids, dtype=torch.long) for ids in tokenized_conversations["input_ids"]]
    targets = [ids.clone() for ids in input_ids]
...
def train():
    ...
    parser = transformers.HfArgumentParser(
        (ChunkFTArguments, DataArguments, MyTrainingArguments)
    )
