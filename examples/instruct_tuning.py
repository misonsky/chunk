...
from chnk import ChunkSeq2SeqTrainer, GetCallBack, peft_function, Seq2SeqTrainer
...
class ChunkFTArguments(ModelArguments):
    ...
    chunk_num: int = field(
        default=1,
        metadata={"help": ("number of parameter chunks")},
    )
    chunk_update_interval: int = field(
        default=1,
        metadata={"help": ("number of optimizer updates to run before switching chunks")},
    )
    chunk_strategy: str = field(
        default="down2up",
        metadata={"help": ("chunk selection strategy of ['down2up','up2down','random']")},
    )
...
def main():
    ...
    if model_args.chunk_tuning:
        trainer = ChunkSeq2SeqTrainer(
            FThandler = GetCallBack(model_args.model_name_or_path),
