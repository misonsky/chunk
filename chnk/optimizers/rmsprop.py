...
class RMSprop(Optimizer):
    ...
    def _init_group(self, group, params_with_grad, grads, square_avgs, momentum_buffer_list, grad_avgs):
        for p in group["params"]:
            ...
            if len(state) == 0:
                state["step"] = 0
                state["square_avg"] = defaultdict(dict)
                if group["momentum"] > 0:
                    state["grad_avg"] = defaultdict(dict)
                if group["centered"]:
                    state["grad_avg"] = defaultdict(dict)
            if counter not in state["square_avg"]:
                state["square_avg"][counter]  = torch.zeros_like(
                    grad, memory_format=torch.preserve_format
                )
            if group["momentum"] > 0 and counter not in state["momentum_buffer"]:
                state["momentum_buffer"][counter] = torch.zeros_like(
                    grad, memory_format=torch.preserve_format
                    )
            if group["centered"] and counter not in state["grad_avg"]:
                state["grad_avg"][counter] = torch.zeros_like(
                        grad, memory_format=torch.preserve_format
                    )
            
            square_avgs.append(state["square_avg"][counter].to(p))

            if group["momentum"] > 0:
                momentum_buffer_list.append(state["momentum_buffer"][counter].to(p))
            if group["centered"]:
                grad_avgs.append(state["grad_avg"][counter].to(p))
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p_index, p in enumerate(group["params"]):
                key_id = self.get_state_key(p)
                counter = p.counter
                square_avg_buffer = self.state[key_id]["square_avg"].get(counter)
                self.state[key_id]["square_avg"][counter], _ = offload_optimizer_state_to_cpu(square_avgs[p_index], square_avg_buffer)
                if group["momentum"] > 0:
                    momentum_buffer_cpu = self.state[key_id]["momentum_buffer"].get(counter)
                    self.state[key_id]["momentum_buffer"][counter], _ = offload_optimizer_state_to_cpu(
                        momentum_buffer_list[p_index],
                        momentum_buffer_cpu,
                    )
                if group["centered"]:
                    grad_avg_buffer = self.state[key_id]["grad_avgs"].get(counter)
                    self.state[key_id]["grad_avgs"][counter], _ = offload_optimizer_state_to_cpu(grad_avgs[p_index], grad_avg_buffer)
                with torch.no_grad():
                    p.new_grad = None
