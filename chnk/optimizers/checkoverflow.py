...
class CheckOverflow(object):
    ...
    def has_overflow_serial(self, params):
        for i, p in enumerate(params):
            if p.new_grad is not None and self._has_inf_or_nan(p.new_grad.data, i):
                return True
        return False
...
def clip_grad_norm_(parameters, max_norm, norm_type=2, mpu=None):
    ...
    parameters = list(filter(lambda p: p.new_grad is not None, parameters))
    max_norm = float(max_norm)
    norm_type = float(norm_type)
    if norm_type == inf:
        total_norm = max(p.new_grad.data.abs().max() for p in parameters)
        ...
        for p in parameters:
            if mpu is not None:
                if (mpu.get_model_parallel_rank() == 0) or is_model_parallel_parameter(p):
                    param_norm = p.new_grad.data.norm(norm_type)
                    total_norm += param_norm.item()**norm_type
            else:
                param_norm = p.new_grad.data.float().norm(norm_type)
                total_norm += param_norm.item()**norm_type
    ...
    if clip_coef < 1:
        for p in parameters:
            p.new_grad.data.mul_(clip_coef)
    return total_norm
...
def get_grad_zeros(parameters, mpu=None):
    ...
    for p in parameters:
        ...
        count_zeros = p.new_grad.numel() - torch.count_nonzero(p.new_grad)
...
def clip_gradients(parameters, max_norm=1.0, global_grad_norm=None, mpu=None, eps=1e-6):
    ...
    if clip_coef < 1:
        for p in parameters:
            p.new_grad.detach().mul_(clip_coef)
