    ...
    def _init_group(
        ...
        for p in group['params']:
            if p.new_grad is not None:
                ...
                if strategy:
                    if len(upd_ran)>0:
                        params_with_grad.append(p[:,upd_ran[0]:upd_ran[1]])
                    else:
                ...
                if len(state) == 0:
                    ...
                    if group['amsgrad']:
                        state['max_exp_avg_sq'] = defaultdict(dict)
                if counter not in state["exp_avg"]:

                    state['exp_avg'][counter] = torch.zeros_like(grad, memory_format=torch.preserve_format)
                    # Exponential moving average of squared gradient values
                    state['exp_avg_sq'][counter] = torch.zeros_like(grad, memory_format=torch.preserve_format)
                    if group['amsgrad']:
                        # Maintains max of all exp. moving avg. of sq. grad. values
                        state['max_exp_avg_sq'][counter] = torch.zeros_like(grad, memory_format=torch.preserve_format)
                        max_exp_avg_sqs.append(state['max_exp_avg_sq'][counter])
                    exp_avgs.append(state['exp_avg'][counter])
                    exp_avg_sqs.append(state['exp_avg_sq'][counter])

                else:

                    exp_avgs.append(state['exp_avg'][counter].to(p))
                    exp_avg_sqs.append(state['exp_avg_sq'][counter].to(p))

                    if group['amsgrad']:
                        max_exp_avg_sqs.append(state['max_exp_avg_sq'][counter].to(p))
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for i, p in enumerate(group["params"]):
                ...
                self.state[key_id]["exp_avg"][counter], _ = offload_optimizer_state_to_cpu(exp_avgs[i], exp_avg_buffer)
                self.state[key_id]["exp_avg_sq"][counter], _ = offload_optimizer_state_to_cpu(exp_avg_sqs[i], exp_avg_sq_buffer)
                if group['amsgrad']:
                    max_exp_avg_sq_buffer = self.state[key_id]["max_exp_avg_sq"].get(counter)
                    self.state[key_id]["max_exp_avg_sq"][counter], _ = offload_optimizer_state_to_cpu(
                        max_exp_avg_sqs[i],
