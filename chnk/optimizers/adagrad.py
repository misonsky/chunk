        ...
        for p in group["params"]:
            if p.new_grad is not None:
                ...
                if len(state) ==0:
                    ...
                    state["sum"] = defaultdict(dict)
                if counter not in state["sum"]:
                    init_value = (
                        complex(self.initial_accumulator_value, self.initial_accumulator_value)
                        if torch.is_complex(p)
                        else self.initial_accumulator_value
                    )
                    state["sum"][counter] = torch.full_like(
                        grad, init_value, memory_format=torch.preserve_format
                    )
                grads.append(grad)
                state_sums.append(state["sum"][counter].to(p))
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p,state_sum in zip(params_with_grad,state_sums):
                key_id = self.get_state_key(p)
                counter = p.counter
                sum_buffer = self.state[key_id]["sum"].get(counter)
                self.state[key_id]["sum"][counter], _ = offload_optimizer_state_to_cpu(state_sum, sum_buffer)
                with torch.no_grad():
                    p.new_grad = None
